...
class SimpliVity:
    ...
    def connect(self, hms_username, hms_password):
        ...
        if(response.status_code == 200):
            ...
            self.headers = {'Authorization':  'Bearer ' + self.access_token, 
                        'Accept' : 'application/vnd.simplivity.v1.9+json'}
            return response
        else:
    ...
    def SetVMPolicy(self, vmname, policyname):
        ...
        return self.doPostData(self.url+'virtual_machinces/set_policy',body)

    def VMmove(self, name, destination):
        body = '{"virtual_machine_name":"'+name+'",\
                 "destination_datastore_id":"'+self.GetDataStoreId(destination)+'"}'
        return self.doPostData(self.url+'virtual_machines/'+self.GetVMId(name)+'/move', body)
    ...
    def VMclone(self, name, cloneName, appConsistent=False, consistencyType='None'):
        ...
        return self.doPostData(self.url+'virtual_machines/'+self.GetVMId(name)+'/clone', body)

    def VMcredentials(self, name, username, password):
        body = '{"guest_username":"'+ username +'",\
                 "guest_password":"'+ password +'"}'
        return self.doPostData(self.url+'virtual_machines/'+self.GetVMId(name)+'/validate_backup_credentials', body)
    ...
    def NewDataStore(self, name, cluster, policy, size):
        ...
        body = '{"name":"'+name+'",\
                ...
                "size":"'+str(size)+'"}'
        return self.doPostData(self.url+'datastores', body)
    ...
    def ResizeDataStore(self, name, size):       
        ...
        return self.doPostData(self.url+'datastores/'+self.GetDataStoreId(name)+'/resize',body)         
        
    def SetDataStorePolicy(self, name, policy):         
        body = '{"policy_id":"'+ self.GetPolicyId(policy) +'"}'
        return self.doPostData(self.url+'datastores/'+self.GetDataStoreId(name)+'/set_policy', body)        
    ...
    def SetClusterTimeZone(self, name, timezone):      
        ...
        return self.doPostData(self.url+'omnistack_clusters/set_time_zone', body)
    ...
    def BackupVM(self, name, destination, retention=0, appConsistent=False, consistencyType='None'):      
        ...
        body='{\
                ...
                "consistency_type":"'+ consistencyType +'",\
                "retention":'+ retention +'}'
        return self.doPostData(self.url+'virtual_machines/'+self.GetVMId(name)+'/backup', body) 
    ...
    def RestoreVM(self, vmname, destination, bckid, restore=False):       
        if restore:
            ...
            return self.doPostData(self.url+'backups/'+bckid+'/restore?restore_original=false', body)
    ...
    def DeleteBackup(self,bckid):       
        body='{\
                "backup_id":["'+ bckid +'"]}'
        return self.doPostData(self.url+'backups/delete', body)
    ...
    def DefinePolicy(self, policyname):       
        ...
        return self.doPostData(self.url+'policies', body)
    ...
    "consistency_type": "'+ consistencyType +'"\n\
        ...
        return self.doPostData(self.url+'policies/'+policy_id+'/rules?replace_all_rules='+str(replace), body)
